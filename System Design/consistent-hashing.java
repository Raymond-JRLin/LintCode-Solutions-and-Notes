...
public class Solution {
    ...
    private List<List<Integer>> method1(int n) {
        // write your code here
        if (n == 0) {
            return null;
        }
        ...
        PriorityQueue<Range> pq = new PriorityQueue<>(new Comparator<Range>() {
            ...
            public int compare (Range r1, Range r2) {
                // return the largest range: descending by range
                if (r2.to - r2.from > r1.to - r1.from) {
                    return 1;
                }
                if (r2.to - r2.from < r1.to - r1.from) {
                    return -1;
                }
                // same range: return small id
                return r1.id - r2.id;
            }
        });
        // insert the first machine
        Range first = new Range(1, 0, 359);
        pq.offer(first);
        for (int i = 1; i < n; i++) {
            // start from 1, if 0, then the first machine would be duplicated
            Range range = pq.poll();
            // split
            Range prev = new Range(range.id, range.from, (range.to + range.from) / 2);
            Range next = new Range(i + 1, (range.to + range.from) / 2 + 1, range.to);
            pq.offer(prev);
            pq.offer(next);
        }
        while (!pq.isEmpty()) {
            ...
            machine.add(range.from);
            machine.add(range.to);
            machine.add(range.id);
            result.add(machine);
        }
        return result;
    }
    ...
    public List<List<Integer>> method2(int n) {
        ...
        for (int i = 1; i < n; i++) {
            int index = 0;
            for (int j = 1; j < i; j++) {
                if (result.get(j).get(1) - result.get(j).get(0) > result.get(index).get(1) - result.get(index).get(0)) {
                    index = j;
                }
            }
            // found the machine to be split
            int from = result.get(index).get(0);
            int to = result.get(index).get(1);
            // update old machine with its 'to' range
            result.get(index).set(1, (from + to) / 2);
            // create new machine
            List<Integer> newMachine = new ArrayList<>();
            newMachine.add((from + to) / 2 + 1);
            newMachine.add(to);
            newMachine.add(i + 1);
